...
function buildReport(snapshot = {}) {
  ...
  const sectorLine = sectors.slice(0, 8).map((s) => `${s.sector || s.name}: ${s.score ?? "--"}`).join(" / ") || "等待板块源刷新";
  ...
  return {
    ...
    sections: [
      ...
      { id: "03", title: "宏观环境", body: "重点观察 10Y / 2Y 美债、DXY、黄金、油价、Fed 降息预期。若美债与美元同步上行，高估值科技股追涨质量下降。" },
      { id: "04", title: "板块表现 / 主题风格", body: sectorLine },
      { id: "05", title: "市场宽度与参与度", body: breadth.explanation || "当前以板块扩散、异动数量、机会榜数量和相对成交量作为宽度代理；真实 A/D、52周新高新低、均线参与度可继续接入。" },
