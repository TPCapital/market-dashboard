...
import { buildNarrativeEngine } from "../lib/narrative-engine.js";
...
const SOURCE_PLANS = {
  ...
  // v4: Political Insider Layer — medium-freq TTL (15 min), push-based via Cron
  politicalTrades: { primary: "QuiverQuant Congressional Trading (SWR, 15min TTL)", backups: ["Structural fallback snapshot"] }
};
...
export async function buildSnapshot(req) {
  ...
  // v4: Fetch political trades layer (SWR, 15-min TTL from Redis)
  let politicalTradesLayer = { status: "snapshot", signals: [], bullishTickers: [], bearishTickers: [], narrative: "", confidence: "LOW", fallback: true };
  try {
    politicalTradesLayer = await buildPoliticalTradesLayer();
  } catch (error) {
    console.error("[SNAPSHOT ENGINE ERROR] politicalTrades", error?.message || error);
  }

  // v4: Build narrative engine (links regime + political trades + sectors)
  let narrativeEngineData = { headline: "", narrative: "", boostedTickers: [], resonanceItems: [], regime: "NEUTRAL" };
  try {
    narrativeEngineData = buildNarrativeEngine({
      marketRegime,
      strategySummary: {},
      politicalTrades: politicalTradesLayer,
      sectorFlows: sectorData,
      premarketMomentum: premarketMomentumLayer,
      confidenceScore,
      indices: marketData.data?.indices || []
    });
  } catch (error) {
    console.error("[SNAPSHOT ENGINE ERROR] narrativeEngine", error?.message || error);
  }
  ...
  try {
    signalEngine = buildSignalEngine({
      ...
      insider: [],
      relativeVolume: relativeVolumeLayer.data?.leaders || [],
      politicalTrades: politicalTradesLayer,        // v4
      narrativeEngine: narrativeEngineData          // v4
    });
  } catch (error) {
  ...
  const snapshot = {
    ...
    layers: {
      ...
      // v4 layers
      politicalTrades: politicalTradesLayer,
      narrativeEngine: narrativeEngineData
    },
    // v4: top-level political trades and narrative for easy frontend access
    politicalTrades: politicalTradesLayer,
    narrativeEngine: narrativeEngineData,
    sources: {
        ...
        newsCatalysts: newsCatalystsSource,
        // v4
        politicalTrades: source("politicalTrades", politicalTradesLayer, politicalTradesLayer.status || "snapshot", generatedAt, "Political Insider Layer", {
          confidence: politicalTradesLayer.confidence || "LOW",
          fallback: politicalTradesLayer.fallback ?? true,
          latency: politicalTradesLayer.latency || null,
          sourcePlan: sourcePlanFor("politicalTrades")
        })
    }
  };
