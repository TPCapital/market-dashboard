...
export default async function handler(req, res) {
  ...
  try {
    ...
    const data = await Promise.all(series.map(async (id) => {
      try {
        const payload = await fetchJson(`https://api.stlouisfed.org/fred/series/observations?series_id=${encodeURIComponent(id)}&api_key=${encodeURIComponent(token)}&file_type=json&sort_order=desc&limit=2`, { timeoutMs: 10000 });
